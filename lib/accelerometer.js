...
const Controllers = {
  ...
  MPU6050: {
    initialize: {
      value(options, callback) {
        ...
        Drivers.get(this.board, "MPU6050", options).on(
          "data",
  ...
  BNO055: {
    initialize: {
      value(options, callback) {
        ...
        const { Drivers } = require("./sip");
        Drivers.get(this.board, "BNO055", options).on(
          "data",
          ({ accelerometer }) => callback(accelerometer)
        );
      },
      ...
      value(options, callback) {
        ...
        if (odr === -1) {
          throw new RangeError(
            "Invalid odr. Expected one of: 800,  400,  200,  100,  50,  12,  6"
          );
        }
  ...
  LIS3DH: {
    ...
    initialize: {
      value(options, callback) {
        ...
        let divider = [16380, 8190, 4096, 1365][range];
        ...
        let threshold = [80, 40, 20, 10][range];
        ...
        // Acceleration
        state.expander.i2cReadOnce(
          address,
          this.REGISTER.CTRL_REG1,
          1,
          (data) => {
            let ctrl1 = data[0];
            ...
            state.expander.i2cRead(
              address,
              this.REGISTER.OUT_X_L | 0x80,
              6,
              (data) => {
                callback({
                  x: int16(data[1], data[0]),
            ...
            state.expander.i2cWrite(
              address,
              this.REGISTER.CLICK_THS,
              threshold
            );
            state.expander.i2cWrite(
              address,
              this.REGISTER.TIME_LIMIT,
              timelimit
            );
            state.expander.i2cWrite(
              address,
  ...
  LSM303C: {
    initialize: {
      value(options, callback) {
        const { Drivers } = require("./sip");
        Drivers.get(this.board, "LSM303C", options).on(
          "data",
          ({ accelerometer }) => callback(accelerometer)
        );
      },
      ...
      this.initialize(options, (data) => {
        ...
        Object.keys(data).forEach((axis) => {
          ...
          if (
            ...
            sensor.calibration.length < calibrationSize
          ) {
            const axisIndex = axes.indexOf(axis);
            ...
            state.zeroV[axisIndex] =
              sum(sensor.calibration) / sensor.calibration.length;
            if (axis === aZ) {
