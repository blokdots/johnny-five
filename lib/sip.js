  ...
  SI1145: {
    ...
    REGISTER: {
      value: {
        ...
        IRQMODE2: 0x06,
        HW_KEY: 0x07,
        MEAS_RATE0: 0x08, // turn off auto mode,
        MEAS_RATE1: 0x09,
        PSALS_AUTO: 0x0F,
        UCOEF0: 0x13,
        UCOEF1: 0x14,
    ...
    initialize: {
      value(board, options) {
        ...
        const handler = (data, type) => {
          ...
          const compVal = uint16(data[0], data[1]) / 100;
          console.log(type, data, compVal)
          // console.log( type, data, compVal )
          
          switch( type ){
            case 'uv':
              const uvIndex = compVal / 100;
              computed.uv = uvIndex;
              break;
            case 'ir':
              computed.ir = compVal;
    ...
    // initialize: {
    //   value(options) {
    //     Components.call(this, "SI1145", options);
    //   }
    // },
    // components: {
    //   value: [SUNLIGHT]
